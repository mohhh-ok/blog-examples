...
| 月間本数 (chars) | ElevenLabs Scale | Cartesia PAYG | Fish Audio | Cloud Run L4 + Qwen3 (※) |
|---|---|---|---|---|
| 10,000本 (2M) | $330 | $100 | **$30** | $756 (固定) / $5〜10 (batch) |
| 30,000本 (6M) | $990 (overage) | $300 | **$90** | $756 (固定) / $15〜25 (batch) |
| 100,000本 (20M) | ~$2,670 (Business + overage) | $1,000 | **$300** | $756 (固定) / **$50〜80** (batch) |

100k本/月で **ElevenLabs vs Fish ≒ 9x** の差。年間 ~$28,000 浮く可能性。

※ Cloud Run L4 GPU + Qwen3-TTS 0.6B self-host、Tokyo region。**固定** = `min-instances=1` で常時 warm (interactive SaaS 向け)、**batch** = scale-to-zero + concurrency 6 batching (非同期 job 向け)。詳細は [`qwen3-tts-notes.md`](./qwen3-tts-notes.md) の「Cloud Run GPU 運用コスト試算」を参照。**300k 本/月** で固定モードが Fish に逆転、**non-interactive job なら全レンジで圧勝** (per req 0.025〜0.05 円)。
...
5. **データ通過リージョン / 提供元の管轄法域** が顧客コンプラ要件に整合 (B2B 主体時のみ。下節参照)

(1) を満たさない provider は不採用。(1) を満たす中で (2)(3)(4)(5) を満たすものを採用候補。
...
> ⚠ 下記は **2026-06 時点の training 知識ベースで未検証**。本採用前に各社の公式 privacy policy / DPA / data residency 設定で再確認すること。特に Fish Audio の拠点・データリージョンは採用判断の肝なので一次情報必須。

### API provider 別の管轄法域

| Provider | 提供元拠点 | データ通過先 (想定) | リスク | 備考 |
|---|---|---|---|---|
| ElevenLabs | 米 + 英 | 米/EU | 低 | 米国輸出規制 (Entity List 顧客への提供制限) のみ |
| Cartesia | 米 | 米 AWS | 低 | startup 故の事業継続リスクは別軸 |
| **Fish Audio** | **中国 (要確認)** | **中国本土 (要確認)** | **高** | PIPL / データセキュリティ法 / 国家情報法。B2B 失注リスク |
| PlayHT | 米 | 米 | 低 | |
| Azure Personal/Custom Voice | Microsoft (米) | Japan East 等選択可 | 低 | Limited Access 申請 + consent flow が前提 |
| Resemble.ai | カナダ | 米/加 | 低 | 5-eyes 圏 |

### Self-host weights 側

- **Fun-CosyVoice 3 / Qwen3-TTS** はどちらも **Alibaba (中国) 発**。ただし self-host (重みローカル DL + ローカル推論) なら**生成データは中国側に流れない**ため、技術的な越境リスクは消える
- 残るのは (a) 重み配布元 (HuggingFace / ModelScope) 規制リスク (b) 顧客知覚リスク (「中国製 AI を採用しない」という調達基準) (c) 米国輸出規制で Qwen 系が将来 Entity List 入りする可能性
- 対策: **重みを社内 artifact registry にミラー**、契約・アーキ図で「ローカル推論で中国サーバに通信しない」と明記

### Alibaba 系を DashScope API 経由で使う場合

Qwen3-TTS / CosyVoice は Alibaba Cloud DashScope (Model Studio) で API 提供もある。この場合は **self-host とは別物**で:

- 音声 / テキストが中国本土の Alibaba サーバを通過 (PIPL 対象)
- API 突然停止リスク (米中規制次第)
- B2B 顧客説明で「中国サーバで合成しています」と言わざるを得ない

本ベンチでは **DashScope API 版は比較対象外**、self-host のみ検証する方針。

### 採用判断への効き方

- **B2C / クリエイター向け SaaS**: 地政学軸はほぼ無視可。コスト最優先で Fish Audio 検討 OK
- **B2B SaaS (金融/官公庁/防衛/通信が顧客に混じる)**: Fish Audio は調達基準で弾かれる業界がある。コスト試算で釣られて選んで後で剥がす事故を防ぐため、判定基準 (5) で自動的に落とす
- **対米事業 (Entity List 対象顧客が含まれる可能性)**: ElevenLabs 等の米国 provider でも顧客側フィルタが必要。Qwen/CosyVoice 系 self-host は将来規制リスクあり
