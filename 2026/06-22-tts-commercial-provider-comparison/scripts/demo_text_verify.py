"""demo target を生成したバリアントを Whisper + F0 で評価。"""
import re
import sys
...
EXPECTED = "結合することができないから矛盾するというのである。"
...
def main() -> None:
    ...
    for wav in sorted(OUT.glob("*.wav")):
        ...
        c = cer(normalize(EXPECTED), normalize(got)) if got else None
        f0 = median_f0(wav)
        if f0 is None:
            print(f"[{wav.stem}] cer={c:.3f} F0=NaN (無音)")
        else:
            diff = f0 - ref_f0
            print(f"[{wav.stem}] cer={c:.3f} F0={f0:.1f} (差 {diff:+.1f})")
        print(f"  got: {got}\n")
